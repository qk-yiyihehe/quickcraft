...
public final class QuickCraftWorkbenchShulkerTelemetry {
    ...
    private static final Map<ServerboundContainerClickPacket, IntegratedActionProbe> integratedServerActions =
            new IdentityHashMap<>();
    ...
    public static void begin(AbstractContainerMenu handler) {
        clear();
        if (handler == null) {
            return;
        }
        active = true;
        sessionSyncId = handler.containerId;
        lastServerRevision = handler.getStateId();
        sessionStartedAtNanos = System.nanoTime();
        windowStartedAtNanos = sessionStartedAtNanos;
        LOGGER.info("工作台潜影盒网络时序开始：containerId={}，初始revision={}", sessionSyncId, lastServerRevision);
    }
    ...
    public static void onClientClickStart(int containerId,
                                          ...
                                          Player player) {
        if (!isCurrentHandler(containerId, player)) {
            return;
        }
        ...
        IntegratedActionProbe integratedProbe = new IntegratedActionProbe(
                kind, containerId, slotId, button, actionType, startedAtNanos);
        startedActions.get().addLast(new StartedAction(
                kind, containerId, slotId, button, actionType, startedAtNanos, integratedProbe));
    }

    public static void onClientClickSent(int containerId,
                                         int slotId,
                                         int button,
                                         ContainerInput actionType,
                                         Player player) {
        ...
        if (action == null || action.packetSent
                || !action.matches(containerId, slotId, button, actionType)
                || !isCurrentHandler(containerId, player)) {
            return;
        }

        long now = System.nanoTime();
        int revision = player.containerMenu.getStateId();
    ...
    public static void onClientClickEnd(int containerId, Player player) {
        Deque<StartedAction> started = startedActions.get();
        StartedAction action = started.pollLast();
        if (action != null && action.packetSent) {
            return;
        }
        if (action != null && isCurrentHandler(containerId, player)) {
            synchronized (integratedTimingLock) {
                integratedDroppedActions++;
            }
        }
    ...
    public static void onIntegratedServerClickStart(ServerboundContainerClickPacket packet) {
        if (!active || packet == null || packet.containerId() != sessionSyncId) {
            return;
        }
        long now = System.nanoTime();
        synchronized (integratedTimingLock) {
    ...
    public static void onIntegratedServerClickProcessed(ServerboundContainerClickPacket packet) {
        if (packet == null) {
            return;
        }
        long now = System.nanoTime();
    ...
    public static void onIntegratedServerClickEnd(ServerboundContainerClickPacket packet,
    ...
    public static void onServerSlotUpdate(int containerId, int revision) {
        onServerUpdate(containerId, revision);
    }

    public static void onServerInventoryUpdate(int containerId, int revision) {
        onServerUpdate(containerId, revision);
    }

    public static void onClientTick(AbstractContainerMenu handler) {
        if (!active || handler == null || handler.containerId != sessionSyncId) {
            return;
        }
    ...
    private static void onServerUpdate(int containerId, int revision) {
        if (!active || !isRelevantSyncId(containerId)) {
            return;
        }
        long now = System.nanoTime();
    ...
    private static boolean isCurrentHandler(int containerId, Player player) {
        return active && player != null && player.containerMenu != null
                && containerId == sessionSyncId && player.containerMenu.containerId == sessionSyncId;
    }

    private static boolean isRelevantSyncId(int containerId) {
        return containerId == sessionSyncId || containerId == -1 || containerId == -2;
    }
    ...
    private static ActionKind classify(int slotId,
        ...
        if (slotId == 0 && actionType == ContainerInput.THROW) {
            return ActionKind.OUTPUT_THROW;
        }
        if (slotId >= 1 && slotId <= 9) {
            return button == 1 && actionType == ContainerInput.PICKUP && isShulker(cursor)
                    ? ActionKind.DIRECT_FILL
                    : ActionKind.GRID_OPERATION;
        }
        if (slotId >= 10) {
            if (actionType == ContainerInput.PICKUP && cursor.isEmpty()) {
                return ActionKind.PICKUP_SOURCE_BOX;
            }
            if (actionType == ContainerInput.PICKUP && isShulker(cursor)) {
    ...
    private static final class StartedAction {
        ...
        private boolean matches(int candidateSyncId,
                                int candidateSlotId,
                                int candidateButton,
                                ContainerInput candidateActionType) {
            return containerId == candidateSyncId && slotId == candidateSlotId
    ...
    private static final class IntegratedActionProbe {
        ...
        private boolean matches(ServerboundContainerClickPacket packet) {
            return containerId == packet.containerId() && revisionAtSend == packet.stateId()
                    && slotId == packet.slotNum() && button == packet.buttonNum()
                    && actionType == packet.containerInput();
        }
    }
