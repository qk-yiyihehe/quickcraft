...
public final class QuickLitematicaFutureCompatBridge {
    ...
    public static void updateBrowserButton(GuiSchematicBrowserBase gui, @Nullable DirectoryEntry entry) {
        ...
        if (browserButton == null) {
            browserButton = gui.addButton(new ButtonGeneric(
                    0, 0, -1, 20,
                    Text.translatable("quickcraft.future_compat.button.short").getString()),
    ...
    private static void openEditor() {
        ...
        if (lastFile == null || lastSnapshot == null || client.currentScreen == null) {
            return;
        }
        client.setScreen(new FutureCompatEditorScreen(
                client.currentScreen, lastFile, lastSnapshot.result()));
    }
}
