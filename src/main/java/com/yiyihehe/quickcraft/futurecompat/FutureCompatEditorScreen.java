 ...
 * 标题左上、按钮右上、列表占满主区域；问题条目源自动带入，目标做当前注册表校验（绿/红描边）；
 * 状态行提供"默认状态"快捷；同名方块/物品保存时自动互补；条目超限折叠提示。
...
public final class FutureCompatEditorScreen extends Screen {
    ...
    private final Map<Row, TextFieldWidget> fields = new HashMap<>();
    private final Map<Row, ButtonWidget> resetButtons = new HashMap<>();
    ...
    public FutureCompatEditorScreen(Screen parent, @Nullable Path schematicFile, ScanResult result) {
        super(Text.translatable("quickcraft.future_compat.editor.title"));
        this.parent = parent;
        ...
        this.snapshot = client.world != null
                ? new VanillaRegistrySnapshot(client.world.getRegistryManager())
                : null;
    }
    ...
    protected void init() {
            ...
            if (line instanceof EntryLine(Row row)) {
                ...
                if (!readOnly) {
                    TextFieldWidget field = new TextFieldWidget(
                            ...
                            this.fieldWidth,
                            18,
                            Text.translatable("quickcraft.future_compat.editor.target"));
                    field.setMaxLength(256);
                    String draft = initialDraft(row);
                    field.setText(draft);
                    field.setChangedListener(text -> this.validate(row, text));
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("quickcraft.future_compat.editor.save"), button -> this.save())
                .dimensions(this.width - MARGIN - 156, 6, 74, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.translatable("quickcraft.future_compat.editor.cancel"), button -> this.close())
                .dimensions(this.width - MARGIN - 76, 6, 74, 20).build());
    }
    ...
    private void buildLines() {
        ...
        if (!this.result.unknownBlocks().isEmpty() || !this.result.unknownItems().isEmpty()) {
            ...
            if (!this.result.unknownBlocks().isEmpty()) {
                this.lines.add(new HeaderLine(
                        Text.translatable("quickcraft.future_compat.editor.group.block", this.result.unknownBlocks().size()).getString(),
                        Text.translatable("quickcraft.future_compat.editor.hint.autopair").getString()));
            }
            for (ScanResult.UnknownBlock block : this.result.unknownBlocks()) {
            ...
            if (itemOnlyCount > 0) {
                this.lines.add(new HeaderLine(
                        Text.translatable("quickcraft.future_compat.editor.group.item", itemOnlyCount).getString(),
                        null));
            }
            for (ScanResult.UnknownItem item : this.result.unknownItems()) {
        ...
        if (!this.result.stateMismatches().isEmpty()) {
            this.lines.add(new HeaderLine(
                    Text.translatable("quickcraft.future_compat.editor.group.state", this.result.stateMismatches().size()).getString(),
                    Text.translatable("quickcraft.future_compat.editor.hint.state").getString()));
        }
        ...
        if (!this.result.invalidMappings().isEmpty()) {
            this.lines.add(new HeaderLine(
                    Text.translatable("quickcraft.future_compat.editor.group.invalid", this.result.invalidMappings().size()).getString(),
                    Text.translatable("quickcraft.future_compat.editor.hint.invalid").getString()));
        }
        ...
        if (userCount > 0) {
            this.lines.add(new HeaderLine(
                    Text.translatable("quickcraft.future_compat.editor.group.user", userCount).getString(),
                    Text.translatable("quickcraft.future_compat.editor.hint.user").getString()));
    ...
    private void buildManagementLines() {
        ...
        this.lines.add(new HeaderLine(
                Text.translatable("quickcraft.future_compat.editor.group.table",
                        blocks.size() + items.size() + states.size()).getString(),
                Text.translatable("quickcraft.future_compat.editor.hint.table").getString()));
    ...
    private void updateResetButton(StateRow stateRow, TextFieldWidget field, int y) {
        boolean differs = !Ids.normalize(field.getText()).equals(Ids.normalize(stateRow.sourceId()));
        ButtonWidget button = this.resetButtons.computeIfAbsent(stateRow, key -> {
            ButtonWidget created = ButtonWidget.builder(
                            Text.translatable("quickcraft.future_compat.editor.default_state"),
                            b -> field.setText(stateRow.sourceId()))
                    .dimensions(this.fieldX - 88, y + 2, 62, 18)
                    .build();
            addDrawableChild(created);
            return created;
        });
    ...
    private void validate(Row row, @Nullable String text) {
        ...
        TextFieldWidget field = this.fields.get(row);
        if (trimmed.isEmpty()) {
            ...
            if (field != null) {
                field.setEditableColor(valid ? 0xFFE0E0E0 : 0xFFFF5555);
            }
        }
        if (row instanceof StateRow stateRow) {
            ButtonWidget resetButton = this.resetButtons.get(row);
            if (resetButton != null) {
    ...
    private void save() {
        if (this.snapshot == null) {
            this.saveError = Text.translatable("quickcraft.future_compat.editor.no_registry").getString();
            return;
        }
        try {
            for (Line line : this.lines) {
            ...
            this.saveError = Text.translatable("quickcraft.future_compat.editor.save_failed").getString();
            return;
        }
    ...
    private void autoPairTarget(String sourceId, String target, boolean fromBlock) {
        for (Line line : this.lines) {
            ...
            TextFieldWidget field = this.fields.get(pair);
            if (field != null && field.getText().trim().isEmpty() && !"invalid".equals(this.validity.get(pair))) {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, this.title, MARGIN, 9, 0xFFFFFFFF);
        if (this.schematicFile != null) {
            context.drawTextWithShadow(this.textRenderer,
                    Text.translatable("quickcraft.future_compat.editor.file",
                            this.textRenderer.trimToWidth(this.schematicFile.getFileName().toString(), this.width / 2)),
                    MARGIN, 25, 0xFF9AA4B2);
        }
        if (this.snapshot == null) {
            context.drawTextWithShadow(this.textRenderer,
                    Text.translatable("quickcraft.future_compat.editor.no_registry"),
                    MARGIN, 41, 0xFFFF5555);
        }
        if (this.saveError != null) {
            context.drawTextWithShadow(this.textRenderer, this.saveError,
                    MARGIN, this.height - 20, 0xFFFF5555);
        }
        if (this.lines.isEmpty()) {
            context.drawTextWithShadow(this.textRenderer,
                    Text.translatable("quickcraft.future_compat.editor.empty"),
                    MARGIN, LIST_TOP + 4, 0xFF8A93A0);
        }
        ...
        for (Line line : this.lines) {
            ...
            switch (line) {
                case HeaderLine(String title, String hint) -> {
                    context.drawTextWithShadow(this.textRenderer, title, MARGIN, y + 5, 0xFFFFD37F);
                    if (hint != null) {
                        int hintX = MARGIN + this.textRenderer.getWidth(title) + 12;
                        context.drawTextWithShadow(this.textRenderer,
                                this.textRenderer.trimToWidth(hint, Math.max(0, this.width - MARGIN - hintX)),
                                hintX, y + 5, 0xFF8A93A0);
                    }
                }
                case EntryLine(Row row) -> {
                    if (hovered) {
                        context.fill(MARGIN, y, this.width - MARGIN, y + ROW_HEIGHT - 2, 0x28FFFFFF);
                    }
                    this.renderRow(context, row, y);
                }
            }
            y += ROW_HEIGHT;
            shown++;
        ...
        if (this.totalLines > this.visibleLines) {
            context.drawTextWithShadow(this.textRenderer,
                    Text.translatable("quickcraft.future_compat.editor.clamped",
                            this.visibleLines, this.totalLines - this.visibleLines),
                    MARGIN, this.height - 34, 0xFFE0A050);
        }
    }

    private void renderRow(DrawContext context, Row row, int y) {
        String source = switch (row) {
            ...
            case AutoMappedRow(String kind, String sourceId, String targetId) ->
                    sourceId + " → " + targetId + kindSuffix(kind);
            case MergedRow(String sourceId) -> sourceId
                    + " (" + Text.translatable("quickcraft.future_compat.marker.merged").getString() + ")";
        };
        if (row instanceof InvalidRow || row instanceof AutoMappedRow) {
            ...
            source += " (" + Text.translatable(originKey).getString() + ")";
        }
        ...
        context.drawTextWithShadow(this.textRenderer,
                this.textRenderer.trimToWidth(source, Math.max(40, textLimit)),
                MARGIN + 4, y + 7, 0xFFD8DEE8);
        context.drawTextWithShadow(this.textRenderer, "→",
                arrowX, y + 7, 0xFF6C7684);
    }

    private static String kindSuffix(String kind) {
        return kind.equals("block")
                ? ""
                : " (" + Text.translatable("quickcraft.future_compat.marker.item").getString() + ")";
    }
